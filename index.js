...
function load_top() {
	fetch("./components/top.html")
		...
		.then((html_data) => {
			...
			console.log(
				is_localstorage == null ? "---ランキングデータはリセットされています---" : "--データあります--"
			);
...
function updateRanking(rankingList, your_score) {
	...
	rankingList.innerHTML =
		...
		challengers_local
			...
			.map(
				(c, index) =>
					`<li class="${c.score == your_score ? "challenge_score" : ""}"><div class="td">${
						index + 1
					}</div><div class="td">${c.name}</div><div class="td">${c.score}</div></li>`
			)
			.join("");
}
